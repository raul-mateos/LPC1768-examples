...
#define PCLK_DAC_MASK       0x00C00000U   // Peripheral clock selection for the Quadrature Encoder Interface
...
// Cambia la definición de la entructura de control del driver a:
...
typedef struct {
  ...
  qadc_handler_t handler;
  void *handler_arg;
} qadc;

// Cambia la definición de la función:
int qadc_initialize(qadc *drv, uintptr_t base_addr) {
  const qadc_Config *cfg;
  int ret;

  Xil_AssertNonvoid(drv != NULL);

  if (drv->is_started == XIL_COMPONENT_IS_STARTED) {
    return 0;
  }
  ...
  qadc_init_cfg(drv, cfg, cfg->base_addr);
  ret = qadc_init_hw(drv);
  return ret;
}
